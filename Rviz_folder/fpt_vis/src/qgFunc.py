...
xml_path = "/home/manipulation-vnc/Code/IP4GP_offline/EKF_V5/allegro_hand_description/UR5_allegro_test.xml"
...
def posrpy2posquat(posrpy):
    """
    Translate pos + rpy to pos + quat
    """
    posquat = np.zeros(7)
    posquat[:3] = posrpy[:3]
    posquat[3:] = Rotation.from_euler('xyz', posrpy[3:]).as_quat()
    return posquat
...
def posrpy2T(posrpy):
    ...
    Return: T mat.
    """
    posrpy = np.ravel(posrpy)
    T = np.mat(np.eye(4))
    pos = np.mat(posrpy[:3]).T
    rpy = posrpy[3:]
    R = Rotation.from_euler('xyz', rpy).as_matrix()
    T[:3, :3] = R
    T[:3, 3] = pos
