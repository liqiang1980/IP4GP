...
def posquat2T(posquat):
    ...
    r = R.from_quat(quat)  # Normalize the quat
    rot = r.as_dcm()
    tform = np.eye(4)
    tform[0:3, 0:3] = rot
...
def posquat2poseuler(posquat):
    ...
    poseuler[3:] = Rotation.from_dcm(T0[:3, :3]).as_euler('zyx')
    return poseuler
...
# def vec2rpy(vec):
#     """
#     Calculate RPY from vector
#     """
#     rpy = Rotation.from_rotvec(vec).as_euler('xyz')
#     return rpy
...
def euler2R(euler):
    ...
    Input euler (zyx), return their R mat in the same frame.
    """
    R = Rotation.from_euler('zyx', euler).as_dcm()
    return R
...
def get_taxel_poseuler(taxel_name, xml_root):
    """
