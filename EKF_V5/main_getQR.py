...
class ekfDataClass:
    ...
    def __init__(self):
        ...
        self.Q = np.mat(np.eye(35)) * 0.000001  # Noise parameter matrix of state xt. Parameter adjustment required
        ...
        # self.R = self.R
        # self.R = np.mat(np.eye(24))
        # self.R = np.mat(np.ones([24, 24])) * 1500
...
class MyClass:
    ...
    def main_process(self):
        ...
        self._dataClass.error_all = self._dataClass.error_all[1:]
        """
        Calculate R and Q mat
        """
        RR = np.matmul(self._dataClass.error_all.T, self._dataClass.error_all)
        print("    >R?", RR)
        np.savetxt('./saveQR/R.txt', RR)

        q_tmp = self._dataClass.GD_all - self._dataClass.xt_all
        QQ = np.matmul(q_tmp.T, q_tmp)
        np.savetxt('./saveQR/Q.txt', QQ)
    ...
    def posterior(self, T_O_W, T0, T1, T2, T3, F_part, tip_flag_outer):
        ...
        print("$$ error:", error.T)
        self._dataClass.error_all = np.vstack((self._dataClass.error_all, error.T[0]))
