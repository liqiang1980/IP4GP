...
class MyClass:
    def __init__(self, txt_dir, dataClass, pass_taxel):
        ...
        self.hand_description_plus = URDF.from_xml_file(
            self.this_file_dir + "/allegro_hand_description/allegro_hand_tactile_v1.4.urdf")  # for Jacobian
        self.hand_description = URDF.from_xml_file( self.this_file_dir +
        "/allegro_hand_description/allegro_hand_right.urdf")  # for FK
        self.hand_tree = kdl_tree_from_urdf_model(self.hand_description)  # for FK
        self.hand_tree_plus = kdl_tree_from_urdf_model(self.hand_description_plus)  # for Jacobian
    ...
    def Jacobian_dealer(self, tip_flag_outer, joint_pos, pos0, pos1, pos2, pos3, index_taxel_name, mid_taxel_name, ring_taxel_name, thumb_taxel_name):
        jstate = joint_pos
        """ Hand to finger tips Jacobians: """
        pos0 = np.ravel(pos0)  # bias, taxel in palm frame {P}
        pos1 = np.ravel(pos1)  # bias, taxel in palm frame {P}
        pos2 = np.ravel(pos2)  # bias, taxel in palm frame {P}
        pos3 = np.ravel(pos3)  # bias, taxel in palm frame {P}

        J_index = np.mat(np.zeros([6, 4]))
        J_mid = np.mat(np.zeros([6, 4]))
        J_ring = np.mat(np.zeros([6, 4]))
        J_thumb = np.mat(np.zeros([6, 4]))
        if tip_flag_outer[0] == 1:
            print("\n KDL_chain: palm_link to " + index_taxel_name)
            index_kdl_kin = KDLKinematics(self.hand_description_plus, "palm_link", index_taxel_name)
        ...
        if tip_flag_outer[1] == 1:
            print("\n KDL_chain: palm_link to " + mid_taxel_name)
            mid_kdl_kin = KDLKinematics(self.hand_description_plus, "palm_link", mid_taxel_name)
            # J_mid = copy.deepcopy(self.mid_kdl_kin.jacobian(q=jstate[4:8], pos=pos1))
            J_mid = copy.deepcopy(mid_kdl_kin.jacobian(q=jstate[4:8]))
            J_mid = np.mat(J_mid)
            print(" J_mid:", J_mid, "\n")
        if tip_flag_outer[2] == 1:
            print("\n KDL_chain: palm_link to " + ring_taxel_name)
            ring_kdl_kin = KDLKinematics(self.hand_description_plus, "palm_link", ring_taxel_name)
            ...
            print(" J_ring:", J_ring, "\n")
        if tip_flag_outer[3] == 1:
            print("\n KDL_chain: palm_link to " + thumb_taxel_name)
            thumb_kdl_kin = KDLKinematics(self.hand_description_plus, "palm_link", thumb_taxel_name)
        ...
        # test_xyzrpy = np.array([0.002, 0, 0.0157, 0, jstate[3], 0])
        # testA = qg.posrpy2T(test_xyzrpy)
        # test_colJ = qg.cal_col_in_J(testA)
        # print("  +++  J_index:", J_index, test_colJ)
    ...
    def main_process(self):
        ...
        mean_t = (self.all_data[-1, 0] - self.all_data[0, 0]) / (self.all_data.shape[0] - 1)
        print("   ??mean_t:", mean_t)
        for i in range(self.all_data.shape[0]):
            ...
            if i > 50:
                test_rpy = np.mat(object_posrpy[3:])
                test_R = R.from_euler('xyz', test_rpy).as_dcm()[0]
                test_quat = R.from_dcm(test_R).as_quat()
                test_vec = qg.R2vec(test_R)
                # test_R2 = qg.vec2R(test_normal)
                # test_normal2rpy = qg.vec2rpy(test_normal)
                # test_quat2 = R.from_euler('xyz', test_rpy).as_quat()
                # print('OK \n', test_quat, '\n', test_quat2, '\n', test_rpy, '\n', test_normal2rpy, '\n', test_R, '\n', test_R2)

                test00 = R.from_dcm(self.T_palm_in_W[:3, :3].T).as_quat()
                quat_test = qg.quaternion_multiply(test00, test_quat)
                quat_test_rpy = R.from_quat(quat_test).as_euler('xyz')
                R_test = np.matmul(self.T_palm_in_W[:3, :3].T, test_R)
                R_test_rpy = R.from_dcm(R_test).as_euler('xyz')
                vec_test = np.matmul(self.T_palm_in_W[:3, :3].T, np.mat(test_vec).T)
                vec_test = qg.normalization(vec_test).T
                vec_test_rpy = R.from_rotvec(vec_test).as_euler('xyz')
                print("\n quat test:", quat_test_rpy, '\n R test:', R_test_rpy, '\n vec test:', vec_test_rpy)
            ...
            Get twist of object
            """
            GD_now = object_posrpy
            delta_GD = GD_now - GD_last
            GD_last = GD_now
            ...
            print("  %%  PASS   ", self.pass_taxel)
            name_taxel = np.delete(name_taxel, np.where(name_taxel == np.mat(self.pass_taxel).T)[-1])
            print("  %%  taxel :", name_taxel)
            ...
            increasing_taxel_name0 = np.empty(0)
            increasing_taxel_name1 = np.empty(0)
            increasing_taxel_name2 = np.empty(0)
            increasing_taxel_name3 = np.empty(0)
            ...
            for jj in range(name_taxel.shape[0]):  # Traverse all taxel in round i
                ...
                # Get pos, euler, R of taxels
                pos, euler = qg.get_taxel_poseuler(taxel_name=name_taxel_tmp, xml_root=self.xml_root)  # Taxel in tip
            ...
            if total_taxel != 0:
                ...
                J_index, J_mid, J_ring, J_thumb = self.Jacobian_dealer(tip_flag_outer, joint_pos, pos_taxel_palm0, pos_taxel_palm1,
                                                                       pos_taxel_palm2, pos_taxel_palm3, index_taxel_name, mid_taxel_name, ring_taxel_name, thumb_taxel_name)

                """
                Get Grasp Matrix G in round i
                """
                big_G = np.mat(np.zeros([6, 24]))
                big_G_GD = np.mat(np.zeros([6, 24]))

                p_GD = object_posrpy[:3]  # p_GD: The real position of object in {W}. Get it from truth value.
                ...
                G_GD0, S_GD0 = qg.get_G(R0, p_GD, c0)
                big_G_GD[:, :6] = G_GD0
                ...
                G_GD1, S_GD1 = qg.get_G(R1, p_GD, c1)
                big_G_GD[:, 6:12] = G_GD1
                ...
                G_GD2, S_GD2 = qg.get_G(R2, p_GD, c2)
                big_G_GD[:, 12:18] = G_GD2
                ...
                G_GD3, S_GD3 = qg.get_G(R3, p_GD, c3)
                big_G_GD[:, 18:] = G_GD3
                ...
                F_part = np.dot(np.linalg.pinv(big_G.T), big_J) * mean_t
                Prediction = np.dot(F_part, joint_vel)
                J_result = np.matmul(big_J, joint_vel) * mean_t
                ...
                self._dataClass.J_result_all = np.vstack((self._dataClass.J_result_all, J_result))

                G_result = np.matmul(big_G_GD.T, delta_GD)
                self._dataClass.G_result_all = np.vstack((self._dataClass.G_result_all, G_result))

                T_object_in_W = qg.posrpy2T(self._dataClass.xt)
                # self.posterior(T_object_in_W, T_taxel_in_W0, T_taxel_in_W1, T_taxel_in_W2, T_taxel_in_W3, F_part, tip_flag_outer)
            # round i is over
        ...
        self._dataClass.J_result_all = self._dataClass.J_result_all[1:, :6]
        self._dataClass.G_result_all = self._dataClass.G_result_all[1:, :6]

        print(" ///////////// G_result: \n", self._dataClass.G_result_all.shape, '\n J_result \n', self._dataClass.J_result_all.shape)
        print('Gap:\n', self._dataClass.G_result_all - self._dataClass.J_result_all)
        """
        Plot
        """
        ...
        # pPlt.plot_4_joint_vel(self._dataClass.joint_vel_all, 'index_joint_vel[deg/s]', 'mid_joint_vel[deg/s]', 'ring_joint_vel[deg/s]', 'thumb_joint_vel[deg/s]')
        # pPlt.plot_4_joint_vel(self.all_data[:, 8:24], 'index_joint_position[deg]', 'mid_joint_position[deg]', 'ring_joint_position[deg]', 'thumb_joint_position[deg]')
        # pPlt.plot_xt_GD_6in1(self._dataClass.J_result_all[:, :6], self._dataClass.J_result_all[:, 6:12],
        #                      label1='dx[mm]', label2='dy[mm]', label3='dz[mm]',
        #                      label4='drx[deg]', label5='dry[deg]', label6='drz[deg]')
    ...
    def get_T_taxel(self, pos, euler, T_tip_in_palm):
        ...
        T_taxel_in_W = np.matmul(self.T_palm_in_W, T_taxel_in_palm)
