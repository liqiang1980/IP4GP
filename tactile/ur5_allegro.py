...
#18拇指大旋转        #19 拇指根部旋转                             #20 拇指大弯曲关节
#21 拇指小弯曲关节   link_15.0_tip  拇指指尖
...
def prediction_model(sim):
    ...
    if (np.array(sensor_data[0:71]) > 0.0).any():
        ...
        save_pose_x_t_xyz = np.append(save_pose_x_t_xyz, np.array(x_t[0, 0:3]), axis=0)
        save_pose_GD_xyz = np.append(save_pose_GD_xyz, np.array([ground_truth[0, 0:3]]), axis=0)
        save_pose_x_t_rpy = np.append(save_pose_x_t_rpy, np.array(x_t[0, 3:6]), axis=0)
