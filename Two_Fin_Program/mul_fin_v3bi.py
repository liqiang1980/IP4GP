...
def ekf_predictor(sim, model, viewer, y_t_update):
    ...
    if (np.array(sim.data.sensordata[0:72]) > 0.0).any() \
        ...
        if not flag:  # get the state of cup in the first round
            init_e = np.hstack((np.random.uniform(-0.005, 0.005, (1, 3)),
                                np.random.uniform(-0.08, 0.08, (1, 3))))  # +-5 mm, +-0.08 rad(4.5 deg)
            # init_e = np.array([-0.005, -0.005, -0.005, -0.08, -0.08, -0.08])
            # init_e = np.array([0.005, 0.005, 0.005, 0.08, 0.08, 0.08])
            init_e = np.array([-0.00083783, 0.00098826, 0.00271985, 0.08, 0.08, 0.08])
...
def index_finger_push():
    global err_all
    err_all = np.loadtxt("./err_twoFin_v3bi.txt")
    ...
    plt_plus.plot_error('Two_Fin_Program/save_i')  # iEKF
...
save_tmp = np.load("save_data/iEKF_incremental.npy")
np.save("save_data/iEKF_incremental.npy", np.vstack((save_tmp, np.hstack((save_pose_y_t_xyz, save_pose_y_t_rpy)))))
print("Run times:", int(save_tmp.shape[0] / 45 + 1))
print("over, shape:", save_tmp.shape)

# np.save("save_data/iEKF_incremental.npy", np.hstack((save_pose_y_t_xyz, save_pose_y_t_rpy)))
