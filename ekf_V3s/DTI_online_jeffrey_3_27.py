...
class AllegroHandController():
    def __init__(self, xml_path):
        ...
        self.joint_states.position[12] = 1.3
        # self.joint_states.velocity = np.zeros(16)

        '''AllegroHand Control Init'''
        self.joint_cmd = JointState()
        self.joint_cmd.header.stamp = rospy.Time.now()
        self.joint_cmd.name = ['joint_{}'.format(i) for i in range(16)]
        self.joint_cmd.position = [0.0] * 16
    ...
    def finger_control_func(self, input_1, input_2, f_part):
        ...
        for cid in ctrl_id[f_name]:
            self.joint_cmd.position[cid] += _input
        # self.joint_cmd.position[12] = 1.3
        # return joint_cmd
