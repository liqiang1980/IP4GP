...
def ekf_predictor(sim, model, viewer, y_t_update):
    ...
    if (np.array(sim.data.sensordata[0:72]) > 0.0).any() \
        ...
        if not flag:  # get the state of cup in the first round
            ...
            init_e = np.array([0.00142373, 0.00406452, - 0.00487179,  0.03151356 , 0.00791485, - 0.02287755])
...
def ekf_posteriori(sim, model, viewer, z_t, h_t, G_big, y_t, F_part, fin_num, fin_tri, coe):
...
def EKF():
    ...
    ######<<<<<<<<<<< Switch fEKF / iEKF <<<<<<<<<<#########
    # y_t_update = y_t
    y_t_update = ekf_posteriori(sim, model, viewer, z_t, h_t, G_contact, y_t, F_part, fin_num, fin_tri, coe)
    ######>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>#########
