...
class basicDataClass:
    def __init__(self, xml_path):
        ...
        # self.data_all = np.loadtxt(self.data_path + 'all_data.txt')[60:400]  # txt:2
...
for iR in range(tac_const.repeatEXP):
    ...
    robctrl = robot_control.ROBCTRL(obj_param=object_param, hand_param=hand_param, model=basicData.model, xml_path=xml_path,
                                    fk=fk)
    ...
    for i, _data in enumerate(basicData.data_all):
        ...
        obj_palm_pos = np.ravel(basicData.world_hand_T[:3, 3].T) + np.ravel(
            np.matmul(basicData.world_hand_T[:3, :3], obj_world_posquat[:3]))
        ...
        if first_contact_flag:  # EKF Start
            ...
            robctrl.interaction(object_param=object_param,
                                ...
                                tacp=tacperception,
                                basicData=basicData)
    """ One experiment over, save data here """
    # print("OVER")
    _data_cur = np.loadtxt("offline_x_state_all.txt")
    data_cur = np.mat(_data_cur)
    # print("data_cur shape:", data_cur.shape, data_cur)
    if iR == 0:
        data_all = [data_cur]
    else:
        _data_all = list(np.load("offline_x_state_repeatEXP.npy"))
        # data_all = np.append(_data_all, data_cur, axis=0)
        _data_all.append(data_cur)
        data_all = _data_all
    np.save("offline_x_state_repeatEXP.npy", data_all)
    # print(" repeat_data shape:", data_all.shape, "  each round: ", data_all.shape[0]/(iR+1))
    print(" repeat_data shape:", np.array(data_all).shape)
